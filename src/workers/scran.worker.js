...
async function unserializeAllSteps(contents) {
    ...
    try {
        ...
        output = {
            ...
            other: {
                custom_selections:
                    superstate.custom_selections.fetchSelections(),
            },
        };
    } finally {
    ...
    if (type == "INIT") {
        ...
        loaded
            .then(async (x) => {
                ...
                try {
                    let collected = await bakana.saveAnalysis(
                        superstate,
                        h5path,
                        {
                            embedded: true,
                    ...
                    for (const [k, v] of Object.entries(payload.inputs.files)) {
                        if ("uid" in v) {
                            if (!(v.uid in preflights)) {
                                ...
                                preflights_summary[v.uid] = await preflights[
                                    v.uid
                                ].summary();
                            }
                            current[k] = preflights[v.uid];
                            summary[k] = summarizeDataset(
                                preflights_summary[v.uid],
                                v
                            );
                        } else {
        ...
        loaded
            .then((x) => {
                ...
                var resp = superstate.marker_detection.fetchGroupResults(
                    cluster,
                    rank_type,
                    modality
                );
        ...
        loaded
            .then((x) => {
                let rank_type = payload.rank_type.replace(/-.*/, ""); // summary type doesn't matter for pairwise comparisons.
                var resp = superstate.custom_selections.fetchResults(
                    payload.cluster,
                    rank_type,
                    payload.modality
                );
        ...
        loaded
            .then((x) => {
                ...
                if (annot.startsWith(`${code}::QC::`)) {
                    ...
                    vec =
                        superstate.cell_filtering.fetchFilteredAnnotations(
                            annot
                        );
                } else {
