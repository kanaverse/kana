...
const StackedHistogram = (props) => {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .call(d3.axisBottom(x).ticks(5).tickValues(d3.extent(data)));
    ...
    const allCells = histogram(
      data.filter((d, i) => {
        return clusters?.[i] === 0;
      })
    );
    const clusterCells = histogram(
      data.filter((d, i) => {
        return clusters?.[i] === 1;
      })
    );
    const allCellsMax = d3.max(allCells, (d) => {
      return d.length;
    });
    const clusterCellsMax = d3.max(clusterCells, (d) => {
      return d.length;
    });
    ...
    svg
      ...
      .attr("height", (d) => {
        return y(-d.length / allCellsMax) - height / 2;
      })
      .style("fill", "#D3D3D3")
      .style("opacity", 0.6);

    svg
      .selectAll("rectCluster")
      .data(clusterCells)
      .enter()
      .append("rect")
      .attr("x", 1)
      .attr("transform", (d) => {
        return `translate(${x(d.x0)}, ${y(d.length / clusterCellsMax)})`;
      })
      .attr("width", (d) => {
        return x(d.x1) - x(d.x0) - 1;
      })
      .attr("height", (d) => {
        return height / 2 - y(d.length / clusterCellsMax);
      })
      .style("fill", color)
      .style("opacity", 0.6);
